...
!	CREATE THE FREQUENCY GRID
	call makegrid		    ! Defined in the makegrid.f file
        ...
        infot=infot+info
        allocate(qlog(-Nm:Nm),stat=info)
        infot=infot+info
        allocate(glog(-Nm:Nm),stat=info)
        infot=infot+info
...
!       Guess self-energy
	...
	if(uniform_grid==1) then
  	   do i=-N,N
	     glog(i)=dlog(dabs((i+1.5d0)/(i+0.5d0)))
	     qlog(i)=1.d0-(i+0.5d0)*dlog(dabs((i+1.5d0)/(i+0.5d0)))
	   end do
	else
	   qlog=0.d0
	   glog=0.d0
...
!	************************************************************
	  ...
	  external funcv1,broydn !,broyden
	  include 'Glob_cons'
...
!	************************************************************
	...
	external funcv1,Lutt_val,broydn  !,broyden
	logical check
	include 'Glob_cons'
...
! 	BEGIN DMFT SELF CONSISTENCY LOOPS
	do while(nloop.le.nlmin.or.((conv.gt.tolf).and. &
	  ...
	  if(dabs(ntot-nf_tot_tmp).gt.1.D-4) then
	    muin(1)=mu_c  				
            call broydn(muin,1,check,funcv1)	! non-linear equation solver 
            mu_c=muin(1)  					! 
	    !tmp_muc=mu_c
...
!		KRAMERS-KRONIG FOR UNIFORM GRID (SEE LATER FOR NON-UNIFORM GRID)
	...
	rho2=0.d0
	do j=-N,N-1
   	  rho1(j)=rhosigma(j+1) - rhosigma(j)
	end do
        rho1(N)=rho1(N-1)
	do j=-N,N
	  rho2(j)=qlog(j)
	...
	do i=llim-1,ulim
	  rlsigma(i)=0.5d0*(rlsg(i)+rlsg(i+1))/(w(1)-w(0))
	end do

	else !		KRAMERS-KRONIG FOR NON-UNIFORM GRID
